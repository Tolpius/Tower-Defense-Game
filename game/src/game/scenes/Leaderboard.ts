...
export class Leaderboard extends Phaser.Scene {
    ...
    private readonly onAuthState = (user: { nickname?: string } | null) => {
        this.currentNickname = user?.nickname ?? null;
        this.displayEntries();
    };
    ...
    private updateFilterButtons() {
        ...
        const allBtn = this.children.getByName("filterBtn_all") as Phaser.GameObjects.Text;
        const normalBtn = this.children.getByName("filterBtn_normal") as Phaser.GameObjects.Text;
        const infiniteBtn = this.children.getByName(
            "filterBtn_infinite",
        ) as Phaser.GameObjects.Text;

        if (allBtn) {
            allBtn.setStyle(this.filterInfinite === undefined ? activeStyle : buttonStyle);
        }
        if (normalBtn) {
            normalBtn.setStyle(this.filterInfinite === false ? activeStyle : buttonStyle);
        }
        if (infiniteBtn) {
            infiniteBtn.setStyle(this.filterInfinite === true ? activeStyle : buttonStyle);
        }
    }
    ...
    private createWorldMapFilter(width: number) {
        ...
        this.add
            ...
            .on("pointerdown", () => {
                this.currentWorldIndex =
                    (this.currentWorldIndex - 1 + this.worldOptions.length + 1) %
        ...
        this.add
            ...
            .on("pointerdown", () => {
                const mapCount = this.getAvailableMaps().length;
                this.currentMapIndex = (this.currentMapIndex + 1) % (mapCount + 1);
                this.updateWorldMapFilter();
                void this.loadLeaderboard();
            });
    ...
    private updateWorldMapFilter() {
        const worldLabel = this.children.getByName("worldFilterLabel") as Phaser.GameObjects.Text;
        const mapLabel = this.children.getByName("mapFilterLabel") as Phaser.GameObjects.Text;
        const mapPrevBtn = this.children.getByName("mapPrevBtn") as Phaser.GameObjects.Text;
        const mapNextBtn = this.children.getByName("mapNextBtn") as Phaser.GameObjects.Text;
        ...
        if (mapPrevBtn) {
            mapPrevBtn.setAlpha(mapsDisabled ? 0.3 : 1);
        }
        if (mapNextBtn) {
            mapNextBtn.setAlpha(mapsDisabled ? 0.3 : 1);
        }
    }
    ...
    private clearEntryTexts() {
        const toDestroy: Phaser.GameObjects.GameObject[] = [];
        this.children.each((child) => {
            if (child.name?.startsWith("entry_") || child.name === "noEntries") {
    ...
    private displayEntries() {
        ...
        this.entries.forEach((entry, index) => {
            ...
            const isCurrentUser = !!this.currentNickname && entry.nickname === this.currentNickname;
            ...
            this.add
                .text(150, y, entry.nickname, { fontSize: "16px", color })
                .setName(`entry_${index}_player`);
            this.add
                .text(400, y, entry.wave.toString(), { fontSize: "16px", color })
                .setName(`entry_${index}_wave`);
            this.add
                .text(500, y, entry.kills.toString(), { fontSize: "16px", color })
                .setName(`entry_${index}_kills`);
            this.add
                .text(600, y, entry.score.toString(), { fontSize: "16px", color })
